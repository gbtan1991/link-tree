      ...
      <div className="grid place-content-center mt-12">
        <ul className="flex flex-col justify-center gap-5">
          {codingProfiles.map((profile) => (
            <li
              key={profile.id}
              className="flex bg-blue-900 font-theme hover:bg-zinc-500 text-zinc-50 hover:transform transition-transform duration-100 ease-in-out hover:scale-110 hover:ease-linear rounded-full shadow-retro"
            >
              <a
                href={profile.link}
                target="_blank"
                className={`flex flex-row items-center gap-5 text-xl py-5 px-14 w-full rounded-full`}
              >
